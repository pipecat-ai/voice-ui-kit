...
source:
  ...
  - package/src/templates/Console/KeypadToggle.tsx
...
<TypeTable
  ...
  type={{
    ...
    noDTMF: {
      description: "Disables the DTMF keypad toggle in the header entirely.",
      type: "boolean",
      required: false,
      default: "false",
    },
    keypadMode: {
      description: 'How the DTMF keypad dispatches presses. `"buffered"`: digits accumulate in an editable field and are sent together on submit, so a mistyped digit can be corrected before anything is transmitted. `"immediate"`: each key sends its tone right away, suited for navigating a live IVR menu.',
    ...
    onClient: {
      description: "Callback fired when a new client instance is created. Useful for subscribing to client events before connecting. Passes through to `PipecatAppBase`.",
