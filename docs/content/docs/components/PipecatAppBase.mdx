...
Unlike the WebRTC/WebSocket transports, MoQ does not need a `/api/offer` or `startBot` endpoint — the browser dials the relay directly. Pass the relay URL via `transportOptions`:
