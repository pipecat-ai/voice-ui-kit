...
<Callout>
By default, the component will automatically call `initDevices()` when mounted. To prevent automatic device initialization, set the `noAutoInitDevices` prop to `true` and manually call `initDevices()` when you're ready to request device permissions.
</Callout>
...
<TypeTable
  ...
  type={{
    ...
    noAutoInitDevices: {
      description: "Whether to prevent automatic initialization of devices. When set to true, the component will not automatically call initDevices(). This prop is passed down to the underlying DeviceDropDown component via deviceDropDownProps.",
      type: "boolean",
      required: false,
      default: "false",
    },
