...
changelog:
  - version: "v0.10.0"
    date: "2026-04-30"
    changes:
      - "`ConversationProvider` is no longer required. Conversation state is now provided by `PipecatClientProvider` from `@pipecat-ai/client-react`."
  - version: "v0.6.0"
    date: "2025-12-15"
    changes:
...
- `useConversationContext` hook to inject messages into the conversation store

It must be used within:
- `PipecatClientProvider` context (for client access, connection state, and conversation state)
