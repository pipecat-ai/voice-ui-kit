...
#### Display Bot Transcripts

Display transcripts from the remote bot:
...
Display transcripts from the local user:
...
- `RTVIEvent.BotOutput` - Receives BotOutput events with spoken text (when RTVI 1.1.0+ is detected)
- `RTVIEvent.BotTtsText` - Receives text chunks as the bot speaks (when BotOutput is not supported)
- `RTVIEvent.BotStoppedSpeaking` - Triggers when the bot stops speaking
- `RTVIEvent.BotTtsStopped` - Triggers when TTS stops
- `RTVIEvent.BotReady` - Detects RTVI version to determine BotOutput support

The component automatically:
- Detects BotOutput support from RTVI version (requires 1.1.0+)
- Processes BotOutput events for non-word-level TTS (supports sentence, token, code, link, etc. aggregation types)
- Falls back to `BotTtsText` events for older RTVI versions
