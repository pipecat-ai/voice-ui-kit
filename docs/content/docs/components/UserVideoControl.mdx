...
<TypeTable
  ...
  type={{
    ...
    unavailableText: {
      description: "When set, renders a disabled button with this message instead of the toggle and hides the picker. Used for device error states (blocked, in-use, etc.). Distinct from `noVideo`, which is a deliberate consumer opt-out.",
      type: "string",
      required: false,
      default: "undefined",
    },
