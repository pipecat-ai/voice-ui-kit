...
The EventsPanel component automatically integrates with the Pipecat Client SDK. It must be used within a `PipecatClientProvider` context.
...
Display events panel with all RTVI events.
...
- **Real-time Events**: Automatically captures and displays RTVI events
- **Event Filtering**: Search/filter events by event name or message content
- **Auto-scroll**: Automatically scrolls to bottom when new events arrive (if user is at bottom)
- **Event Types**: Captures various event types including:
  - Transport state changes
  - Bot connection/disconnection
  - Bot ready events
  - Speaking events (bot and user)
  - Track events (audio, video, screen)
  - Participant events
  - Error events
  - Server messages
- **Formatted Display**: Shows timestamp, event name, and message for each event
