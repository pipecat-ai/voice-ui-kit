...
changelog:
  - version: "v0.13.1"
    date: "2026-08-25"
    changes:
      - "`BotOutput` events are now deduplicated per segment: repeated emissions for the same segment and spoken status are suppressed."
      - "Dedupe state is reset when the client connects or disconnects."
      - "`BotOutput` log messages now include the segment ID (when available) and spoken status details."
---
...
- **BotOutput**: Bot output events. Word-level events are filtered out. On servers using RTVI 2.0.0+, each segment is logged only when its spoken status changes (deduplication), and messages include the segment ID and spoken status details.
