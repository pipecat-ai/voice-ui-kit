...
Creating a custom theme is easy. You can use the `@custom-variant` directive, override the design tokens and add your own custom classes.

To support scoping, here is the recommended way to create a custom theme:

```css title="custom-theme.css"
@custom-variant theme-custom {
  &:where([data-theme="custom"] *) {
    @slot;
  }
}
@layer base {
  .vkui-root[data-theme="custom"],
  :root[data-theme="custom"]:not(:has(.vkui-root)),
  :root:not(:has(.vkui-root)) [data-theme="custom"] {
...
Apply it by toggling the data-theme attribute (e.g. `data-theme="custom"`):
...
function ScopedRoot() {
  ...
  return (
    <div className="vkui-root" data-theme={resolvedTheme}>
      <YourApp />
    </div>
  );
}
